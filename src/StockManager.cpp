...
{
    auto it = find_if(items.begin(), items.end(), [id](const StockItem &item)
                      { return item.id == id; });
    Table resultTable;
    resultTable.format()
        .font_align(FontAlign::center)
...
void StockManager::searchProductByName(const std::string &keyword) const {
    ...
    if (it != items.end())
    {
        resultTable.add_row({"Search Result for ID: " + to_string(id)});
        resultTable.add_row({"ID", "Type", "Brand", "Model", "Year", "Origin", "Price"});
        stringstream ss_price;
        ss_price << "$ " << fixed << setprecision(2) << it->price;
        resultTable.add_row({to_string(it->id),
                             it->type,
                             it->brand,
                             it->model,
                             to_string(it->year),
